...
```{r setup, include=FALSE}
     ...
     # Safe webshot wrapper to prevent compilation failure if PhantomJS is missing
     safe_webshot <- function(html_path, png_path, ...) {
       if (webshot::is_phantomjs_installed()) {
         webshot::webshot(html_path, png_path, ...)
       } else {
         warning("PhantomJS is not installed. Skipping saving table image to: ", basename(png_path), 
                 ". Run `webshot::install_phantomjs()` to enable HTML-to-PNG table export.")
       }
     }
```
...
#| eval: false
    ...
    save(dat.dis, dat.dis.23, dat.dis.25, file = here("data", "cint-prolific-data1.rda"))
```

# Descriptives

```{r Dislikes Data}
    load(here("data", "cint-prolific-data1.rda"))
...
```{r Dislikes Table}
     kbl(dat.g, caption = "", format = "html", align = c("l", "c", "l", "c"), row.names = F,
          ...
          footnote(general = "N = 1408 (Prolific); N = 959 (Lucid); N = 2367 (All)") |>
          save_kable(here("Tabs", "desc-tab-dislike.html"))
     safe_webshot(here("Tabs", "desc-tab-dislike.html"), here("Tabs", "desc-tab-dislike.png"))
```

```{r Plot Functions}
     make.lolli.plot <- function(w, a = 0, v = 0, h = 0, refline = TRUE, ref = 0, high) {
...
```{r Univariate Table}
     ...
     safe_webshot(here("Tabs", "desc-tab-dislike-dist.html"), here("Tabs", "desc-tab-dislike-dist.png"))
```
...
```{r Bivariate Plots}
     ...
     make.lolli.plot(make.plot.dat(dat.dis, Age, bin.dis), a = 35, v = 1.1, h = 1, ref = r, high = 0.4)
     save_plot(here("Plots", "uni-dist-cat-tol-age.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, Inc, bin.dis), a = 52, v = 1.1, h = 1, ref = r, high = 0.4)
     save_plot(here("Plots", "uni-dist-cat-tol-inc.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, PolViews, bin.dis), a = 55, v = 1.1, h = 1, ref = r, high = 0.4)
     save_plot(here("Plots", "uni-dist-cat-tol-pol.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, Educ, bin.dis), a = 52, v = 1.1, h = 1, ref = r, high = 0.4)
     save_plot(here("Plots", "uni-dist-cat-tol-edu.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, Race, bin.dis)[-5, ], ref = r, high = 0.35) + coord_flip()
     save_plot(here("Plots", "uni-dist-cat-tol-rac.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, Gender, bin.dis)[1:2, ], ref = r, high = 0.25) + coord_flip()
     save_plot(here("Plots", "uni-dist-cat-tol-gen.png"), height = 15, width = w)
     # Graded Intolerance
     r <- mean(dat.dis$num.dis)
     make.lolli.plot(make.plot.dat(dat.dis, Age, num.dis), a = 35, ref = r, high = 6, v = 1.5, h = 1)
     save_plot(here("Plots", "uni-dist-grd-int-age.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, Inc, num.dis), a = 52, v = 1.1, h = 1, ref = r, high = 4)
     save_plot(here("Plots", "uni-dist-grd-int-inc.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, PolViews, num.dis), a = 55, v = 1.1, h = 1, ref = r, high = 4)
     save_plot(here("Plots", "uni-dist-grd-int-pol.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, Educ, num.dis), a = 52, v = 1.1, h = 1, ref = r, high = 4)
     save_plot(here("Plots", "uni-dist-grd-int-edu.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, Race, num.dis)[-5, ], ref = r, high = 4) + coord_flip()
     save_plot(here("Plots", "uni-dist-grd-int-rac.png"), height = 15, width = w)
     make.lolli.plot(make.plot.dat(dat.dis, Gender, num.dis)[1:2, ], ref = r, high = 4) + coord_flip()
...
```{r Regression Analysis}
    ...
    safe_webshot(here("Tabs", "zinf-reg.html"), here("Tabs", "zinf-reg.png"), zoom = 4)
```
