...
```{r Comparing Univariate Distribution Plot}
    ...
    p <- p + ylim(0, 0.25)
    p <- p + scale_color_manual(values = c("blue", "brown"))
    p
...
```{r Zero Inflated Regression}
    ...
    summary(m1)
    tab_model(m1, transform = NULL, digits = 3, prefix.labels = "none",
...
```{r Predicted Probabilities}
    ...
    p
    save_plot(here("Plots", "zip-pred-comp.png"), height = 20, width = 15)
```