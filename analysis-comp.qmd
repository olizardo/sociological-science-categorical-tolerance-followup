...
#| eval: false
    ...
    dat.dis.25 <- dat.genres(dat.25)$dat.g |> 
        ...
        mutate(num.dis = sum(c_across(Classical:Metal), na.rm = TRUE)) |> 
        ungroup() 
    save(dat.dis, dat.dis.25, file = here("data", "cint-prolific-data2.rda"))
```

```{r Getting Data (SSI)}
    dat.dis.12 <- read_dta(here("data", "SSI2012.dta")) |> 
...
```{r Univariate Distribution Data}
    load(here("data", "cint-prolific-data2.rda"))
...
```{r Zero Inflated Regression}
    ...
    safe_webshot(here("Tabs", "zinf-reg-comp.html"), here("Tabs", "zinf-reg-comp.png"), zoom = 4)
```

```{r Predicted Probabilities}
    pv <- predict_response(m1, "sample", type = "zi_prob")
    p <- plot(pv, ci_style = "errorbar", dot_size = 6)
    p <- p + theme_minimal()
