...
\newpage
\begin{abstract}
This paper updates the empirical picture of Categorical Tolerance (CT), namely, the pattern of refusing to report dislikes across cultural genres, for the third decade of the twenty-first century in the United States. Analyzing recent survey data from two platforms, I find that CT has continued its march among Americans, reaching approximately one-fifth of respondents. The analysis confirms earlier-observed demographic trends, showing that CT is strongly associated with younger cohorts and non-white individuals. However, I also find that individuals reporting the highest educational attainment are now overrepresented among categorical tolerants, suggesting that CT may increasingly function as an elite cultural strategy consistent with contemporary forms of status display, signaling openness and refusal to refuse. Furthermore, the study uses political ideology as a predictor of symbolic exclusion, revealing that while the odds of being a CT itself are not strongly polarized, the inclination toward symbolic exclusion among non-CTs is, with conservatives significantly more likely to express a greater volume of cultural dislikes than liberals, showing that political ideology, alongside age and ethnoracial identity, is a central determinant of cultural exclusion in the contemporary context. 

\textbf{Reproducibility Package:} Data files and \textit{R} code (in Quarto Markdown format) necessary to reproduce all of the analyses, tables, and figures reported in the paper can be found at the following GitHub repo: \url{https://github.com/olizardo/sociological-science-categorical-tolerance-followup}.
\end{abstract}
\newpage
...
Table~\ref{tab:dis-by-genres} shows the probability of any genre experiencing a judgment of dislike for both the Prolific and Lucid samples, pooling the 2021 and 2023 Prolific samples. For each genre, participants were asked to ``Tell us your feelings about each type of music.'' Evaluation options were anchored at the extremes with ``dislike a great deal'' and ``like a great deal;'' ``neither like nor dislike'' served as the neutral point. A participant was counted as expressing a dislike if they selected either the ``dislike a great deal'' or the ``dislike somewhat'' options for a particular genre.\footnote{The genres included in previous work that are not included in the present study are Big Band/Swing, Bluegrass, Folk, Gospel, Latin/Mariachi/Salsa, Mood/Easy Listening, and New Age/Space Music.} 

As we can see, Metal is the genre most likely to be disliked in both samples, consistent with previous work on Americans' musical tastes based on earlier data \citep{bryson1996anything-311, lizardo2015musical-8c6}. Both samples also agree that the two genres least likely to be disliked are Rock and Pop music. There is some disagreement across samples on the exact ranking of other genres. Country music experiences the most significant disparity, ranking second among the most disliked genres in the Prolific sample but near the bottom in the Lucid sample. Beyond that, both datasets agree that Musicals, Rap, and Disco are at the top of the dislike list, while Classical, R\&B, and Jazz are closer to the bottom. 
