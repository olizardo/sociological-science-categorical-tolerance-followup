...
\newpage
\begin{abstract}
This paper updates the empirical picture of Categorical Tolerance (CT), namely, the pattern of refusing to report dislikes across cultural genres, in the United States during the third decade of the twenty-first century. Analyzing recent survey data from two platforms, I find that CT has continued its march among Americans, reaching approximately 20\% of respondents. The analysis confirms earlier-observed demographic trends, showing that CT is strongly associated with younger cohorts and non-white individuals. However, I also find that individuals reporting the highest educational attainment are now overrepresented among categorical tolerants, suggesting that CT may increasingly function as an elite cultural strategy consistent with contemporary forms of status display, signaling openness and refusal to refuse. Furthermore, the study uses political ideology as a predictor of symbolic exclusion, revealing that while the odds of being a CT itself are not strongly polarized, the inclination toward symbolic exclusion among non-CTs is, with conservatives significantly more likely to express a greater volume of cultural dislikes than liberals. This result shows that political ideology, alongside age and race, is a central determinant of cultural exclusion in the contemporary context. 
\end{abstract}
\newpage
...
Table~\ref{tab:dis-dist} shows the univariate distribution of the number of dislikes for both the Prolific and Lucid samples. As we can see, both datasets closely agree on the proportion of categorical tolerants, with the Prolific data yielding about 19\% and the Lucid dataset about 20\%. When we combine both, we get an estimate of 19.6\%, suggesting that about a fifth of the American population today can be classified as categorical tolerants, a presumed increase from L\&S's estimate of about 15\% using data from 2012.\footnote{We should be cautious when making direct comparisons, given that the number of genres included is smaller (twelve versus twenty in L\&S's 2012 survey) in both the Prolific and Lucid samples. Nevertheless, we can conclude that categorical tolerance remains a pervasive phenomenon among a substantial number of Americans.} 
...
\begin{figure}[ht!]
    ...
    \caption{Univariate plot of number of dislikes, joint Prolific and Lucid samples (N = 2367).}
    \label{fig:main}
\end{figure}

One worry, however, is that the differences between the percentages of categorical tolerants observed here and those in L\&S's original study are a simple function of the fact that the previous study used a larger number of genres than this one, thus exposing people to the ``risk'' of disliking more options. As a robustness check, I compared the distribution of dislikes in the current sample with that in L\&S's original study. While most of the abstract genre labels used in the two studies can be matched without a problem, two discrepancies need to be reconciled. First, the current research includes ``Disco'' as a genre, one that was absent from the original L\&S study. Accordingly, I drop this genre, leaving us with 11 comparable genres. The other issue is that, while the current research asks only about likes and dislikes for "Rock," L\&S's previous study disaggregated this category into three subgenres: Classic, Contemporary, and Alternative. Accordingly, I merge these three categories in L\&S's original data into an omnibus ``Rock'' category, treating a dislike of any of them as a dislike of Rock. Finally, to compare apples to apples, I use only the more representative data collected in 2025 as a comparison point, as L\&S's data also came from a similar sample weighted to match 2010 census demographics in terms of age, race, and education.
...
\begin{figure}[ht!]
    ...
    \end{subfigure} 
    \hspace{2cm}
      \renewcommand{\thesubfigure}{Figure 3(b)}% New fixed/manual numbering
     \begin{subfigure}[b]{0.3\textwidth}
        \includegraphics[width=1.0\textwidth]{Plots/zip-pred-comp.png}
            \caption{}
            \label{fig:comp-pred}
    \end{subfigure}
    \caption*{}
...
The single coefficient estimate in the ``count'' part of the model in Table~\ref{tab:comp-zip} tests the hypothesis that the differences between the two distributions are due to the non-zero counts. This hypothesis is not supported by the data ($\beta_{2025} = -0.01, p = 0.61$). The single coefficient estimate in the ``inflation'' part of the model tests the hypothesis that the main difference between the two distributions is the larger excess zeros observed in 2025. This hypothesis is supported by the data ($\beta_{2025} = 0.26, p < 0.05$). Furthermore, the predicted probabilities from this model can thus give us a more precise estimate of the different representations of ``true'' categorical tolerants across the two samples.\footnote{Estimated using the \texttt{ggeffects} packages in \textit{R}.} These are shown in Figure~\ref{fig:comp-pred}. As we can see the estimated number of categorical tolerants from the model in Table~\ref{tab:comp-zip} for 2012 is close to that reported in L\&S's original paper: About 15\%. The number of categorical tolerants estimated in the 2025 Cint sample, however, is significantly larger, at just under 19\%.
