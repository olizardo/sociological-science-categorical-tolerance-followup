dat.demog2 <- function(x) {
   w <- x %>% 
      ...
      mutate(ext.Educ = if_else(Educ.n == 1 | Educ.n == 6, 1, 0)) %>% 
      mutate(polviews.n = as.numeric(PolViews)) %>% 
      mutate(Moderate = if_else(polviews.n == 4, 1, 0))  %>% 
      mutate(ext.pol = if_else(polviews.n == 1 | polviews.n == 7, 1, 0)) 
      return(w)
}